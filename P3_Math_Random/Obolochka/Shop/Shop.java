...
public class Shop{
  ...
  public Product searchComputer(String input_name, boolean v1){
    for(int i = 0; i < this.warehouse.size(); i++){
      if(this.warehouse.get(i).name.equals(input_name) && v1){
        return this.warehouse.get(i);
      }
    }
    return new Product();
  ...
  public String searchComputer(String input_name, boolean v1, boolean v2){
    String searching = "";
    int lenn = 0;
    for(int i = 0; i < this.warehouse.size(); i++){
      if(this.warehouse.get(i).index.equals(input_name) && v1 && v2){
        searching+="\nТовар: " + this.warehouse.get(i).name + "\n"
        + "Индекс товара: " + this.warehouse.get(i).index + "\n"
        + "Стоимость: " + this.warehouse.get(i).cost + "\n";
        return searching;
